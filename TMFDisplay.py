...
Version 2.3 > 2.4 / 05.07.2025
Added "Predicted Finish Time" display. Most useful for maps with many checkpoints.
...
def pft_updater(current_pft, current_checkpoint):
    global displayed_pft, displayed_sourcepft

    if current_checkpoint == 0:
        display(sourcepft, None, True)
        return
    
    current_pft = prefixpft + format_time(current_pft, formatpft)
    
    if displayed_pft != current_pft or displayed_sourcepft != sourcepft:
        displayed_pft = current_pft
        displayed_sourcepft = sourcepft
        display(sourcepft, current_pft, None)
...
def updater():
    ...
    if display_toggle:
        ...
        if enabledpft and cp0timedisplay:
            display(sourcepft, None, spectator)
    ...
    if enabledcptime or enabledpft:
        current_mstime = read_address_value(final_addresses[0], False)
    if enabledcp or enabledcptime or enabledrespawns or enabledpft:
        current_checkpoint = read_address_value(final_addresses[1], False)
    if enabledcp or enabledcptime or enabledpft:
    ...
    if enabledpft:
        if current_checkpoint > 0:
            pft_updater(current_mstime / current_checkpoint * current_max_checkpoint, current_checkpoint)
        else:
            pft_updater(None, 0)
    if enabledrespawns:
...
def setup(*args): #Get PID > Get Base Address > Alt client and In-Game check > Get final addresses.
    ...
    if display_toggle:
        ...
        if enabledpft:
            display(sourcepft, None, disabled_displays)
        if enabledrespawns:
...
def script_defaults(settings):
    ...
    obs.obs_data_set_int(settings, "formatpft", 0)
    ...
    obs.obs_data_set_bool(settings, "enabledpft", False)
    ...
    obs.obs_data_set_string(settings, "sourcepft", "No Source")
    obs.obs_data_set_string(settings, "prefixpft", "Finish Time: ")
...
def options_update(props, prop, *settings):
    ...
    property_list.append(p_statuspft := obs.obs_properties_get(props, "statuspft"))
    ...
    property_list.append(p_enabledpft := obs.obs_properties_get(props, "enabledpft"))
    property_list.append(p_sourcepft := obs.obs_properties_get(props, "sourcepft"))
    property_list.append(p_prefixpft := obs.obs_properties_get(props, "prefixpft"))
    property_list.append(p_formatpft := obs.obs_properties_get(props, "formatpft"))
    property_list.append(p_examplesourcepft := obs.obs_properties_get(props, "examplesourcepft"))
    property_list.append(p_examplepft := obs.obs_properties_get(props, "examplepft"))
    ...
    if text_sources_list:
        source_list = [p_sourcecp, p_sourcecptime, p_sourcepft, p_sourcerespawns, p_sourcegear, p_sourcerpm, p_sourcefps, p_sourceservertimer]
    ...
    enabledpft = obs.obs_data_get_bool(settingscopy, "enabledpft")
    sourcepft = obs.obs_data_get_string(settingscopy, "sourcepft")
    prefixpft = obs.obs_data_get_string(settingscopy, "prefixpft")
    formatpft = obs.obs_data_get_int(settingscopy, "formatpft")
    
    if not sourcepft:
        sourcepft = "No Source"
    obs.obs_data_set_string(settingscopy, "examplesourcepft", sourcepft)
    obs.obs_data_set_string(settingscopy, "examplepft", prefixpft + format_time(5025678, formatpft))
    #-Predicted Finish Time End-#
    ...
    status_source = [
        ...
        (sourcepft, "statuspft", enabledpft),
        (sourcerespawns, "statusrespawns", enabledrespawns),
        (sourcegear, "statusgear", enabledgear),
        (sourcerpm , "statusrpm", enabledrpm),
        (sourcefps, "statusfps", enabledfps),
        (sourceservertimer, "statusservertimer", enabledservertimer)
    ]
    ...
    if s_option == "Status":
        ...
        obs.obs_property_set_visible(p_statuspft, True)
    ...
    elif s_option == "Predicted Finish Time":
        obs.obs_property_set_visible(p_enabledpft, True)
        obs.obs_property_set_visible(p_sourcepft, True)
        obs.obs_property_set_visible(p_prefixpft, True)
        obs.obs_property_set_visible(p_formatpft, True)
        obs.obs_property_set_visible(p_examplesourcepft, True)
        obs.obs_property_set_visible(p_examplepft, True)
...
def script_properties():
    ...
    obs.obs_property_list_add_string(p, "Predicted Finish Time", "Predicted Finish Time")
    ...
    p = obs.obs_properties_add_text(props, "statuspft", "Predicted Finish Time:", obs.OBS_TEXT_INFO)
    ...
    #-Predicted Finish Time-#
    p = obs.obs_properties_add_bool(props, "enabledpft", "Enabled")
    obs.obs_property_set_modified_callback(p, options_update)
    p = obs.obs_properties_add_list(props, "sourcepft", "Source", obs.OBS_COMBO_TYPE_LIST, obs.OBS_COMBO_FORMAT_STRING)
    obs.obs_property_set_modified_callback(p, options_update)
    
    p = obs.obs_properties_add_text(props, "prefixpft", "Prefix", obs.OBS_TEXT_DEFAULT)
    obs.obs_property_set_modified_callback(p, options_update)

    p = obs.obs_properties_add_list(props, "formatpft", "Time Format", obs.OBS_COMBO_TYPE_LIST, obs.OBS_COMBO_FORMAT_INT)
    obs.obs_property_list_add_int(p, "1:23:45.67", 3)
    obs.obs_property_list_add_int(p, "1:23:45", 2)
    obs.obs_property_list_add_int(p, "83:45.67", 1)
    obs.obs_property_list_add_int(p, "83:45", 0)
    ...
    p = obs.obs_properties_add_text(props, "examplesourcepft", "Source:", obs.OBS_TEXT_INFO)
    p = obs.obs_properties_add_text(props, "examplepft", "Example:", obs.OBS_TEXT_INFO)
    #-Predicted Finish Time End-#
